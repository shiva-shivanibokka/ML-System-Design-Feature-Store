...
def compute_features(
    client=None, snapshot_time: datetime | None = None, feature_version: str = "v1"
) -> int:
    client = client or get_duckdb_client()
    return compute_and_store(
        client, snapshot_time or datetime.utcnow(), feature_version
    )


def get_latest_features_for_entities(
    entity_ids: list[int], feature_version: str = "v1", as_of: datetime | None = None
) -> pd.DataFrame:
...
def get_feature_stats(
    feature_version: str = "v1", since_days: int = 7
) -> dict[str, dict]:
    client = get_duckdb_client()
